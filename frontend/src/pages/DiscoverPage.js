// pages/DiscoverPage.js — v10
// Spoilage Risk Assessor integrated → auto-routes map to optimal storage
...
function RiskBadge({level}){const c=RISK_CFG[level]||RISK_CFG.SAFE;return <span style={{display:"inline-flex",alignItems:"center",gap:4,background:c.bg,border:`1px solid ${c.border}`,color:c.color,padding:"2px 8px",borderRadius:20,fontSize:11,fontWeight:700}}>{c.icon} {c.label}</span>;}
...
function SpoilageRiskPanel({onRiskResult,spoilageMeta}){
  ...
  const [form,setForm]=useState({crop_type:"tomato",harvest_age_hrs:"48",distance_km:"20",ambient_temp_c:"28",humidity_pct:"65",travel_time_hrs:"2",season_month:String(new Date().getMonth()+1),vehicle_type:"covered"});
  const [loading,setLoad]=useState(false);const [result,setRes]=useState(null);const [error,setErr]=useState("");
  const set=(k,v)=>setForm(f=>({...f,[k]:v}));
  const inp={width:"100%",background:"var(--bg-l)",border:"1px solid var(--bd)",color:"var(--tx)",fontFamily:"var(--fb)",fontSize:"12px",padding:"7px 10px",borderRadius:"8px",outline:"none",boxSizing:"border-box"};
  const lbl={fontSize:"10px",fontWeight:700,color:"var(--tx-m)",textTransform:"uppercase",letterSpacing:".6px",display:"block",marginBottom:3};

  const handleSubmit=async(e)=>{
    e.preventDefault();setLoad(true);setErr("");setRes(null);
    const payload={...form,harvest_age_hrs:parseFloat(form.harvest_age_hrs),distance_km:parseFloat(form.distance_km),ambient_temp_c:parseFloat(form.ambient_temp_c),humidity_pct:parseFloat(form.humidity_pct),travel_time_hrs:parseFloat(form.travel_time_hrs),season_month:parseInt(form.season_month)};
    try{const{data}=await mlAPI.spoilage(payload);setRes(data);onRiskResult?.({...data,crop_type:form.crop_type,distance_km:parseFloat(form.distance_km)});}
    catch{const age=parseFloat(form.harvest_age_hrs)/24,temp=parseFloat(form.ambient_temp_c),score=Math.min(100,Math.round(age*7+Math.max(0,temp-25)*2)),risk_level=score>=70?"CRITICAL":score>=35?"RISKY":"SAFE";const fb={risk_level,risk_score:score,confidence_pct:65,model_used:"rules",recommendations:[risk_level==="CRITICAL"?`⚠️ Urgent: Book cold storage immediately for ${form.crop_type}.`:risk_level==="RISKY"?"Book cold storage within 2 days.":"Produce is safe — compare rates.",temp>28?"Pre-cool before transport.":"Maintain cool conditions."],crop_type:form.crop_type,distance_km:parseFloat(form.distance_km)};setRes(fb);onRiskResult?.(fb);}
  ...
  return(
    <div className="card" style={{padding:18}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
        <div style={{width:32,height:32,borderRadius:8,background:"var(--cp-pale)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,flexShrink:0}}>🌿</div>
        <div>
          <div style={{fontSize:14,fontWeight:800,color:"var(--tx)"}}>Assess Crop Risk</div>
          <div style={{fontSize:11,color:"var(--tx-m)"}}>XGBoost spoilage prediction · auto-routes optimal storage</div>
        </div>
      </div>
      <form onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:10}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:8}}>
          <div><label style={lbl}>Crop Type</label><select style={inp} value={form.crop_type} onChange={e=>set("crop_type",e.target.value)}>{cropList.map(c=><option key={c} value={c}>{c.replace("_"," ").replace(/\b\w/g,l=>l.toUpperCase())}</option>)}</select></div>
          <div><label style={lbl}>Harvest Age (hrs)</label><input type="number" style={inp} min="1" max="5000" value={form.harvest_age_hrs} onChange={e=>set("harvest_age_hrs",e.target.value)}/></div>
          <div><label style={lbl}>Distance (km)</label><input type="number" style={inp} min="0" max="500" value={form.distance_km} onChange={e=>set("distance_km",e.target.value)}/></div>
          <div><label style={lbl}>Travel Time (hrs)</label><input type="number" style={inp} min="0" max="48" step="0.5" value={form.travel_time_hrs} onChange={e=>set("travel_time_hrs",e.target.value)}/></div>
      ...
      {result&&(()=>{const c=RISK_CFG[result.risk_level]||RISK_CFG.SAFE;return(
        <div className="anim-fadeup" style={{marginTop:12,padding:"12px 14px",background:c.bg,border:`1px solid ${c.border}`,borderRadius:10}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            ...
            <div style={{flex:1}}><div style={{fontWeight:800,fontSize:14,color:c.color}}>{c.label}</div><div style={{fontSize:11,color:"var(--tx-m)"}}>Score: {Math.round(result.risk_score)}/100 · Confidence: {result.confidence_pct}%</div></div>
            {result.risk_level==="CRITICAL"&&<div style={{fontSize:11,fontWeight:700,color:c.color,padding:"2px 8px",border:`1px solid ${c.border}`,borderRadius:20}}>⏱ Urgent!</div>}
          </div>
          {result.recommendations?.map((r,i)=><div key={i} style={{fontSize:12,color:"var(--tx)",lineHeight:1.5,padding:"4px 0",borderTop:i>0?"1px solid var(--bd)":"none"}}>{r}</div>)}
          <div style={{marginTop:8,fontSize:11,color:"var(--tx-s)",fontStyle:"italic"}}>
            {result.risk_level==="CRITICAL"||result.risk_level==="RISKY"?"📍 Map now shows nearest cold storages — sorted by distance for fastest delivery.":"📍 Map now shows cheapest cold storages — take your time to compare rates."}
          </div>
        </div>
      );})()}
    </div>
...
export default function DiscoverPage(){
  const[storages,setStorages]=useState([]);const[recommended,setRecommended]=useState([]);const[selectedStore,setSelectedStore]=useState(null);const[bookingTarget,setBookingTarget]=useState(null);const[farmerPos,setFarmerPos]=useState([20.5937,78.9629]);const[loadingStores,setLoadingStores]=useState(true);const[toast,setToast]=useState("");const[lastRefresh,setLastRefresh]=useState(null);const[riskData,setRiskData]=useState(null);const[optimalStorage,setOptimalStorage]=useState(null);const[spoilageMeta,setSpoilageMeta]=useState(null);const[cityFilter,setCityFilter]=useState("");const[stateFilter,setStateFilter]=useState("");

  useEffect(()=>{mlAPI.spoilageMeta().then(({data})=>setSpoilageMeta(data)).catch(()=>{});},[]);

  const fetchStorages=useCallback(async(silent=false)=>{if(!silent)setLoadingStores(true);try{const{data}=await farmerAPI.listStorages({status:"available"});const f=data.storages||[];setStorages(f.length>0?f:DEMO_STORAGES);setLastRefresh(new Date());}catch{setStorages(p=>p.length>0?p:DEMO_STORAGES);}finally{setLoadingStores(false);}},[]); 
  useEffect(()=>{fetchStorages();const iv=setInterval(()=>fetchStorages(true),30000);return()=>clearInterval(iv);},[fetchStorages]);
  useEffect(()=>{if(navigator.geolocation)navigator.geolocation.getCurrentPosition(pos=>setFarmerPos([pos.coords.latitude,pos.coords.longitude]),()=>{});},[]);
  ...
  const handleRiskResult=useCallback((risk)=>{
    ...
    if(risk.risk_level==="CRITICAL"||risk.risk_level==="RISKY"){
      ...
      showToast(`🚨 ${risk.risk_level} — routing to nearest: ${optimal.name}`);
    }else{
      optimal=withDist.sort((a,b)=>parseFloat(a.price_per_kg_per_day)-parseFloat(b.price_per_kg_per_day))[0];
      showToast(`✅ SAFE — routing to most affordable: ${optimal.name}`);
    }
  ...
  const filteredDisplay=baseList
    ...
    .filter(s=>{if(cityFilter&&!s.district?.toLowerCase().includes(cityFilter.toLowerCase()))return false;if(stateFilter&&s.state!==stateFilter)return false;return true;})
  ...
  return(
    <div style={{maxWidth:1280,margin:"0 auto",padding:"24px 20px"}}>
      ...
      <div style={{marginBottom:20}} className="anim-fadeup">
        <h1 style={{fontSize:22,fontWeight:800,color:"var(--tx)",marginBottom:4}}>Discover Cold Storage</h1>
        <p style={{fontSize:13,color:"var(--tx-m)"}}>Assess crop risk first — the map navigates to optimal storage automatically.</p>
      </div>
      ...
      {optimalStorage&&riskData&&(
        <div className="anim-fadeup" style={{marginBottom:16,padding:"12px 16px",background:riskData.risk_level==="SAFE"?"rgba(63,107,51,.07)":"rgba(139,58,43,.07)",border:`1px solid ${riskData.risk_level==="SAFE"?"rgba(63,107,51,.25)":"rgba(139,58,43,.25)"}`,borderRadius:12,display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
          ...
          <div style={{flex:1,minWidth:200}}>
            ...
            <div style={{fontSize:12,color:"var(--tx-m)",marginTop:2}}>{optimalStorage.district}, {optimalStorage.state} · {Number(optimalStorage.distance_km).toFixed(1)} km · ₹{parseFloat(optimalStorage.price_per_kg_per_day).toFixed(2)}/kg/day · {(parseFloat(optimalStorage.available_capacity_kg)/1000).toFixed(1)} MT available</div>
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <RiskBadge level={riskData.risk_level}/>
            <button onClick={()=>setBookingTarget({...optimalStorage,riskPrefilled:riskData})} style={{background:"var(--cp)",color:"var(--bg)",border:"none",borderRadius:8,padding:"8px 16px",fontFamily:"var(--fd)",fontWeight:800,fontSize:13,cursor:"pointer",boxShadow:"0 3px 12px var(--cp-glow)"}}>Book Now →</button>
          </div>
        </div>
      )}
      ...
      <div className="card anim-fadeup d2" style={{padding:"10px 14px",marginBottom:14,display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
        <span style={{fontSize:11,fontWeight:700,color:"var(--tx-s)",textTransform:"uppercase",letterSpacing:".6px",whiteSpace:"nowrap"}}>🔍 Filter:</span>
        <input className="inp" value={cityFilter} onChange={e=>setCityFilter(e.target.value)} placeholder="City / District…" style={{flex:1,minWidth:130,padding:"6px 10px",fontSize:12}}/>
        <select className="inp" value={stateFilter} onChange={e=>setStateFilter(e.target.value)} style={{padding:"6px 10px",fontSize:12,minWidth:130}}><option value="">All States</option>{INDIA_STATES.map(s=><option key={s} value={s}>{s}</option>)}</select>
        {(cityFilter||stateFilter)&&<button className="btn btn-ghost" onClick={()=>{setCityFilter("");setStateFilter("");}} style={{padding:"6px 12px",fontSize:12}}>✕ Clear</button>}
        <span style={{fontSize:11,color:"var(--tx-s)",whiteSpace:"nowrap"}}>{filteredDisplay.length} results</span>
        {lastRefresh&&<span style={{fontSize:10,color:"var(--cp)",fontWeight:600,display:"flex",alignItems:"center",gap:4,whiteSpace:"nowrap"}}><span style={{width:6,height:6,borderRadius:"50%",background:"var(--cp)",display:"inline-block",animation:"ping 1.5s ease-in-out infinite"}}/>Live · {lastRefresh.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}</span>}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 380px",gap:20,alignItems:"start"}}>
        ...
        <div className="card anim-fadeup d4" style={{maxHeight:460,overflowY:"auto",padding:16}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
            <span style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:".6px",color:"var(--tx-m)"}}>{recommended.length?"Optimal Matches":"Nearest Storages"}</span>
            {riskData&&<RiskBadge level={riskData.risk_level}/>}
          </div>
          {loadingStores?(<div style={{display:"flex",flexDirection:"column",gap:10}}>{[1,2,3,4].map(i=><div key={i} className="skel" style={{height:70}}/>)}</div>):(<StorageList storages={filteredDisplay} selectedId={selectedStore?.id||optimalStorage?.id} highlightId={optimalStorage?.id} onSelect={setSelectedStore} onBook={s=>setBookingTarget({...s,riskPrefilled:riskData})}/>)}
        </div>
      </div>

      {bookingTarget&&<BookingModal storage={bookingTarget} riskData={bookingTarget.riskPrefilled||riskData} onClose={()=>setBookingTarget(null)} onSuccess={()=>{setBookingTarget(null);showToast("Booking sent! Operator will confirm shortly.");}}/>}
